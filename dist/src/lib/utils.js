...
async function cloneGithubRepo(repo, destination) {
    if (!shelljs_1.default.which('git')) {
        logger_1.mainLogger.error('Sorry, this script requires "git"');
    ...
    try {
        ...
        logger_1.mainLogger.error('Repo is not available or an unexpected error has occurred. Please try again...');
        shelljs_1.default.exit(1);
    }
    logger_1.mainLogger.error('Cloning to', pathToClone);
    if (shelljs_1.default.exec(`git clone ${repoURI} ${pathToClone}`).code !== 0) {
        logger_1.mainLogger.error('Cannot clone the repo');
        shelljs_1.default.exit(1);
    }
    logger_1.mainLogger.info('Repo cloned');
    ...
    logger_1.mainLogger.warn('IMPORTANT - You need to install dependencies - IMPORTANT');
    logger_1.mainLogger.info('Done, you are ready to go!');
}
...
function deleteAndInitGit(pth) {
    ...
    if (shelljs_1.default.rm('-rf', '.git/').code !== 0) {
        logger_1.mainLogger.error('Cannot delete ".git" folder');
        shelljs_1.default.exit(1);
    }
    logger_1.mainLogger.info('Initializing git');
    if (shelljs_1.default.exec('git init').code !== 0) {
        logger_1.mainLogger.error('"git init" command failed');
        shelljs_1.default.exit(1);
    }
    if (shelljs_1.default.exec('git add .').code !== 0) {
        logger_1.mainLogger.error('"git add" command failed');
        shelljs_1.default.exit(1);
    }
    if (shelljs_1.default.exec(`git commit -am "Auto-commit by Freshland"`).code !== 0) {
        logger_1.mainLogger.error('"git commit" command failed');
        shelljs_1.default.exit(1);
    }
}
...
function updatePackageJSON(projectName, pth) {
    ...
    if (shelljs_1.default.test('-e', './package.json')) {
        logger_1.mainLogger.info('Updating "package.json"');
    ...
    if (shelljs_1.default.test('-e', './package-lock.json')) {
        logger_1.mainLogger.info('Updating "package-lock.json"');
